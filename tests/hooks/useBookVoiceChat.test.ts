...
describe('useBookVoiceChat — partial transcripts', () => {
  it('partials rerun as the latest merge over the accumulated text', async () => {
    const { result } = renderChat();
    await act(async () => {
      await result.current.toggle();
      await flushMicrotasks();
    });
    // After onDefinite mutates the closed-over `accumulated` variable, subsequent
    // onPartial calls will prefix it. This guards against accidental shifts to
    // partial-only accumulation semantics.
    await act(async () => {
      asrHandlers.onDefinite?.('first');
      asrHandlers.onPartial?.('second');
      await flushMicrotasks();
    });
    expect(result.current.partialTranscript).toContain('first');
    expect(result.current.partialTranscript).toContain('second');
  });

  it('onDefinite appends to the accumulated transcript', async () => {
    const { result } = renderChat();
    await act(async () => {
      await result.current.toggle();
      await flushMicrotasks();
